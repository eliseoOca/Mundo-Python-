...
def codificar():
    ...
    if texto != "":
        ...
        mensaje("Error","Texto codificado")
    else:
        mensaje("Error","Error al codificar")
def descodificar():
    texto=textoCodificado.get()
    if texto != "":
        textoSinCodificar.set(descodifica(texto))
        mensaje("Error","Texto descodificado")
    else:
        mensaje("Error","Error al descodificar")
...
etiquetaCodificado=Label(frame,text="Texto sin codificado").grid(row=4,column=1)
